...
const HeroSection = () => {
  ...
  return (
    <section ref={ref} className="relative min-h-screen flex flex-col justify-center pt-32 pb-24 overflow-hidden">
    ...
    <section ref={ref} className="py-28 lg:py-36 relative">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <motion.h2
          ...
          className="text-4xl md:text-5xl lg:text-6xl font-black leading-[1.05] tracking-tight text-foreground mb-20 max-w-5xl"
        >
        ...
        {/* Two-column text */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border rounded-3xl overflow-hidden mb-12">
          {[
            <>
            ...
            <motion.div key={i}
              ...
              className="p-10 bg-background text-muted-foreground text-lg leading-relaxed"
            >
              {content}
            </motion.div>
          ))}
...
const MissionSection = () => {
  ...
  const missionPoints = [
    { icon: TrendingUp, title: 'Lasting Value Creation', description: "Creating lasting value across the agribusiness chain while strengthening Morocco's position as a global benchmark." },
    { icon: Leaf, title: 'Sustainable Growth', description: 'Supporting Moroccan growers, social standards, and long-term farming sustainability for ethical and resilient growth.' },
    { icon: Heart, title: 'Social Impact', description: 'Giving back to society and investing in agribusiness development to build global impact that benefits all.' },
  ];
    ...
    <section ref={ref} className="py-28 lg:py-36 relative bg-primary/5">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-20">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.15 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {missionPoints.map((point, index) => (
            <motion.div key={index}
              ...
              className="group relative overflow-hidden rounded-2xl p-8 border border-border bg-card hover:border-primary/50 transition-all duration-500 hover:-translate-y-1"
            >
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                style={{ background: 'radial-gradient(ellipse at 20% 20%, hsl(88,92%,30%,0.1), transparent 65%)' }} />
              <div className="relative z-10">
                <div className="w-12 h-12 rounded-xl bg-primary/15 flex items-center justify-center text-primary mb-6 group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-400">
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3">
              {visionPillars.map((item, index) => (
                <div key={index} className="group p-4 rounded-xl border border-border bg-card hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 text-center">
                  <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mb-2">{item.label}</div>
